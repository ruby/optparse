...
When an option is found, its name and the given argument are yielded.
...
When an option with an argument is found, its name and the given argument are yielded.
...
and the option name is yielded with no argument.

  $ ruby optional_argument.rb -x
  "--xxx "
